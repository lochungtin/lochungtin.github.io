...
def render_image(block: dict) -> str:
    ...
    if caption or label:
        ...
        caption_html = f'<p class="proj-image-caption">{label_part}{e(caption)}</p>'
...
def render_bullet_list(block: dict) -> str:
    ...
    return f"{title_html}" f'<ul class="{css_class}">\n' f"{lis}" "</ul>\n"
...
def render_numbered_list(block: dict) -> str:
    ...
    if style == "steps":
        ...
        return f"{title_html}" '<div class="proj-steps">\n' f"{rows}" "</div>\n"
        ...
        return f"{title_html}" '<ul class="proj-next-list">\n' f"{lis}" "</ul>\n"
        ...
        return f"{title_html}" '<ol class="proj-numbered--boxed">\n' f"{lis}" "</ol>\n"
        ...
        return f"{title_html}" '<ol class="proj-numbered--simple">\n' f"{lis}" "</ol>\n"
...
def render_challenges(block: dict) -> str:
    ...
    return '<div class="proj-challenges">\n' f"{cards}" "</div>\n"
...
def build_sidebar(sections: list) -> str:
    ...
    for sec in sections:
        sid = e(sec.get("id", ""))
        title = e(sec.get("title", ""))
        links_html += f'<a href="#{sid}" class="proj-sidebar-link" data-section="{sid}">{title}</a>\n'
...
def build_footer(meta: dict) -> str:
    ...
    prev_html = (
        f'<a href="{e(prev["url"])}" class="proj-footer-sibling">{e(prev["label"])}</a>'
        if prev
        else ""
    )
    next_html = (
        f'<a href="{e(nxt["url"])}"  class="proj-footer-sibling">{e(nxt["label"])}</a>'
        if nxt
        else ""
    )
